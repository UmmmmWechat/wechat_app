...
//        int limit = (lastIndex == NumberUtil.ALL_INDEX) ? Integer.MAX_VALUE : lastIndex + NumberUtil.PAGE;
////
////        List<Spot> spots = spotRepository.findByProvinceAndCityAndRegion(location.province, location.province, location.province, limit);
////
////        if (spots.size() >= limit)
////            return spots.subList(lastIndex, limit);
////
////        List<Spot> moreSpots = spotRepository.findByProvinceAndCity(location.province, location.province, location.province, limit - spots.size());
////
